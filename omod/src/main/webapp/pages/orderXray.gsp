...
<script type="text/javascript">
    var breadcrumbs = [
        ...
        { label: "${ui.message("radiology.task.orderXray.label")}" }
    ];
</script>
...
<div id="contentForm">
    <h1>${ ui.message("radiology.orderXray.title") }</h1>
    <form action="${ ui.actionLink("radiology", "radiologyRequisition", "orderXray") }" data-bind="submit: isValid">
        ...
        <div class="left-column">
            <label for="study-search"><strong>${ ui.message("radiology.orderXray.studySearchInstructions") }</strong></label><br/>
            <input id="study-search" type="text"
                   ...
                   placeholder="${ ui.message("radiology.orderXray.studySearchPlaceholder") }"/>
        </div>

        <div class="right-column">
            <div class="row">
                ${ ui.includeFragment("emr", "field/radioButtons", [
                        label: ui.message("radiology.order.timing"),
                        formFieldName: "urgency",
                        options: [
                                [ value: "ROUTINE", label: ui.message("radiology.order.timing.routine"), checked: true ],
                                [ value: "STAT", label: ui.message("radiology.order.timing.urgent") ]
                        ]
                ]) }
            </div>
            <div class="row">
                <p><strong>${ ui.message("radiology.orderXray.portableQuestion") }</strong></p>
                <p>
                    <input type="checkbox" class="field-value" value="portable" data-bind="checked: portable"/>
                    <span>${ ui.message("radiology.yes") }</span>
                    <select name="examLocation" data-bind="enable:portable, options:locations, optionsText:'name', optionsValue:'id', optionsCaption:'${ ui.escapeJs(ui.message("radiology.orderXray.examLocationQuestion")) }', value:portableLocation">
                    </select>
                </p>
            </div>
        </div>
        ...
        <div class="left-column">
            ${ ui.includeFragment("emr", "field/textarea", [ label:"<strong>" + ui.message("radiology.order.indication") + "</strong>", formFieldName: "clinicalHistory", labelPosition: "top", rows: 5, cols: 35 ]) }
        </div>

        <div class="right-column">
            <div data-bind="visible: selectedStudies().length == 0">
                <span style="color: blue;">${ ui.message("radiology.orderXray.noStudiesSelected") }</span>
            </div>
            <div data-bind="visible: selectedStudies().length > 0">
                <label>${ ui.message("radiology.orderXray.selectedStudies") }</label>
