...
<script type="text/template" id="radiologyStudyDetailsTemplate">
    <h2>{{- procedure}}</h2>
    <small>Accession Number:</small><span class="report-value">{{- accessionNumber }}</span><br/>
    <small>Performed by:</small><span class="report-value"> {{- technician }}</span><br/>
    <small>Performed on:</small><span class="report-value"> {{- datePerformed }}</span><br/>
    <small>Images available:</small><span class="report-value"> {{- imagesAvailable ?  '${ ui.message("emr.yes")}' :  '${ ui.message("emr.no")}' }}</span><br/>
    <br/>
{{_.each(reports, function(report) { }}
<span class="radiology-report">
    <small>Reported by:</small><span class="report-value"> {{- report.principalResultsInterpreter }}</span><br>
    <small>Report date:</small><span class="report-value"> {{- report.reportDate }}</span><br>
    <small>Report type:</small><span class="report-value"> {{- report.reportType }}</span>
